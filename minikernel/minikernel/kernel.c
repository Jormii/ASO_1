...
static void liberar_proceso()
{
	...
	for (int descriptor = 0; descriptor != NUM_MUT_PROC; ++descriptor)
	{
		...
		{
			cerrar(descriptor, p_proc_actual->descriptores_mutex[descriptor]);
		}
	}
...
int sis_lock_mutex()
{
	...
	{
		printk("\tEl proceso %d está intentando hacer lock sobre el mutex %d, el cual ya estaba bloqueado\n", p_proc_actual->id, descriptor);
	...
	case MUTEX_TIPO_RECURSIVO:
		...
		printk("\tNumero de locks realizados sobre el mutex con descriptor %u: %d\n", descriptor, mutex_lock->num_locks);
		break;
	case MUTEX_TIPO_NO_RECURSIVO:
		if (mutex_lock->estado == MUTEX_ESTADO_BLOQUEADO)
		{
			printk("\tError: El mutex no recursivo con descriptor %u ya habia sido bloqueado\n", descriptor);
			return -2;
		}
		break;
	default:
	...
	printk("\tSe ha realizado lock sobre el mutex %d\n", descriptor);
	return 0;
}
...
static BCP *buscar_procesos_con_mutex(mutex *mutex)
{
	...
	while (p_proc != NULL)
	{
		for (int i = 0; i != NUM_MUT_PROC; ++i)
		{
			...
			{
				if (p_proc->descriptores_mutex[i]->id_proc_bloq == mutex->id_proc_bloq)
				{
					return p_proc;
				}
			}
...
static void unlock(int descriptor, mutex *mutex_unlock)
{
	...
	if (proceso_desbloquear != NULL)
	{
		...
		printk("El mutex con descriptor %d e id %d que pertenecia al proceso %d ahora pertenece a %d, el cual ha sido desbloqueado\n",
			   descriptor, mutex_unlock->mutex_id, p_proc_actual->id, proceso_desbloquear->id);
	}
	else
	{
	...
	printk("\tSe ha realizado unlock sobre el mutex con descriptor %d e id %d\n", descriptor, mutex_unlock->mutex_id);
}
